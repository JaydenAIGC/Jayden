...
class Backend:
    ...
    def gen_subject_ideas(self):
        try:
            ...
            _land=_idea_r.choice(["平缓草甸","平缓草甸","辽阔草原","辽阔草原","薄雾丘陵","薄雾丘陵","晨雾旷野","晨雾旷野","雾霭林地","雾霭林地","浅雾原野","薄雾山麓","寂静湖面","静谧湖畔","暮色荒滩","朦胧海岸","云海","沙漠","积雪丘陵","薄雾林间"])
...
只写【核心主体+配套环境】，80-120字。可以包含光线时段或天气状态（如黄昏、清晨、正午、阴天、薄雾等），但不要写画风、镜头、构图、人物、画质——那些由出图系统统一添加。

主体构造准则：
- ⚠️ {count}条必须使用**完全一致的句式结构**（主语位置、修饰顺序、收尾方式相同），只轻微变化视角对应的环境细节
- ⚠️ {count}条必须使用**完全一致的美术画风**
- **重点描述建筑外观形态（轮廓/比例/表面特征/空间姿态），而非抽象尺寸**
...
- 完整保留以下主体：{subject}
- **环境：根据地貌关键词自动匹配——主体含海/水/湖→水体环境，含雪/冰→雪原，含沙/漠→荒漠，含林/草/原/丘→草甸原野，其他→云海。均质连续无零散杂物，远景融入雾气**
- 全程用中文"""
        else:
...
- 自主创作宏大核心主体，单一核心不堆砌
- **环境：根据地貌关键词自动匹配——含海/水/湖→水体，含雪/冰→雪原，含沙/漠→荒漠，含林/草/原/丘→草甸原野，其他→云海/荒原。均质连续无零散杂物，远景融入雾气**
- 全程用中文"""
        try:
            h=self._headers("llm")
            p={"model":self.config.get("text_model","deepseek-v4-flash"),
...
**核心红线（绝对遵守）：**
- **不新增任何原始文本不存在的物体、建筑、景观、环境元素（人物除外——人物由AI优化统一管理）**
- **不修改原有主体、场景基调**

**人物规则（AI优化统一管理，原始人物描述不信任一律删掉重写）：**
- 若原始有或无人物 → 统一由AI结合场景氛围自主匹配风格统一的单人形象（如骑马/古风长袍/科幻战甲/探险装束/远行者等），放置画面下方远景，体型渺小作为尺度参照，人物不能占据视觉中心
- 硬性限制：仅限单人，远景，不特写，不生成人群，不生成写实清晰五官

**优化逻辑（【第三步：智能重构二次精细化优化】）：**
1. 禁止随意增删核心画面元素
2. **场景逻辑验证（必须做）：根据场景基调检查所有元素是否匹配——海岸场景不能出现荒漠/雪地元素，黄昏场景不能出现正午烈日，室内不能出现自然环境；发现矛盾立即修正或删除**
3. 镜头自由自适应，清除矛盾镜头参数，维持大面积留白
4. 构图自主选用平衡规整样式，杜绝失衡凌乱构图
5. 保留远景单人约束，仅限单人、禁止特写与人群
6. 理顺语句、剔除重复冗余词汇，强化宏大空灵氛围
7. 画面干净通透，单一主光源，剔除杂乱彩光；材质规避锈蚀、斑驳风化
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        # ② 画风标签
        if is_natural:
            style_tag="美术概括化的3A写实游戏场景CG静帧，遵循PBR三维渲染逻辑；写实仅作为基底，主动概括简化细节脱离照片真实感；无手绘笔触，依靠体积雾与统一色调塑造辽阔静谧氛围；低纹理密度哑光平滑材质，自然光线柔和过渡，低饱和度统一色调，画面干净克制"
        else:
            style_tag="美术概括化的3A写实游戏场景CG静帧，遵循PBR三维渲染逻辑；写实仅作为基底，主动概括简化细节脱离照片真实感；无手绘笔触；低纹理密度哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，轻薄大气透视薄雾——近景通透仅远景朦胧消融，无厚重块状体积雾，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景，画面干净克制"
        # ③ 核心主体 从 desc 来
        # ④ 场景自适应环境 — 柔和弥散、边界渐变的均质自然基底
        # 注意：地貌类关键词优先于水体类，避免"深海裂谷"被误判为水下
        if any(w in d for w in ["荒漠","沙漠","戈壁","干旱","裂谷","沙地"]):
            env="，主体立于无垠荒漠之上，平整地面向远方渐变，地平线隐入薄雾"
        elif any(w in d for w in ["雪原","冰原","冻土","苔原"]):
            env="，主体立于无垠雪原之上，雪地平整向远方渐变，地平线消融于灰白雾霭中"
        elif any(w in d for w in ["森林","林","树","丛林","密林","林地","草甸","草地","草原","丘陵","原野","旷野","平原"]):
            env="，主体立于平缓均质草甸之上，远方雾霭弥漫，远景渐溶于雾气之中"
        elif any(w in d for w in ["湖泊","湖面","湖畔","静水"]):
            env="，主体矗立于静谧湖畔，宽阔平静水面倒映天光，远景消融于薄雾之中"
        ...
        if not has_light:
            ...
            light_color="低饱和统一色调，色彩克制，无杂乱多重光斑，均匀通透光线"
        # ⑦ 画质+光学
