...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        if is_natural:
            ...
            style_tag="美术概括化的3A写实游戏场景CG静帧，遵循PBR三维渲染逻辑；写实仅作为基底，主动概括简化细节脱离照片真实感；无手绘笔触；低纹理密度哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，平缓渐变体积雾由近至远景物逐步消融，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景，画面干净克制"
        # ③ 核心主体 从 desc 来
        ...
        # 均质基底统一收尾 — 体积雾层次
        env+="，平缓渐变体积雾，近景雾气稀薄不遮挡主体轮廓，由近至远雾气浓度缓慢提升，远景景物逐步消融融入雾霭，无杂乱块状浓雾无穿透光束，雾气色调统一"
