...
- 轮廓简洁干净，无锯齿繁杂外缘
- ⚠️ **核心审美：质感对冲**——环境柔软混沌（云雾/雾霭/水体/薄雾）包裹建筑硬朗清晰轮廓，模糊弥散vs规整坚硬，全部美感来源于此对比
- 完整保留以下主体：{subject}
...
- **氛围核心：辽阔、孤静、超然，人在宏大造物面前的渺小与沉默。安静、停滞、近乎永恒。巨型人造构造独立于自然演化体系之外，不属于自然，与自然生态互不交融。空灵静谧，万物静止无声，无活跃生命迹象但非末日废墟，画面干净通透有呼吸感**
- 全程用中文"""
        else:
...
- 画面层次仅三层，**氛围核心：辽阔孤静超然，巨型造物独立于自然体系之外，与自然互不交融，安静停滞近乎永恒**
- 全程用中文"""
        try:
            h=self._headers("llm")
            p={"model":self.config.get("text_model","deepseek-v4-flash"),
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        if is_natural:
            ...
            style_tag="影视环境概念原画，Octane渲染，写实物理渲染，极简巨型几何构造，依靠嵌套、悬挑、高低层级塑造层次感，低纹理密度，哑光平滑材质，剔除繁杂细碎雕刻、密集零件，单一柔和漫射天光，低饱和度统一色调，画面大面积留白，强烈人与巨构尺度反差，辽阔孤静超然氛围，安静停滞近乎永恒，巨型人造构造独立于自然演化体系之外不属于自然，质感对冲：柔软混沌环境包裹硬朗清晰几何巨构，轮廓干净利落，8K超清"
        # ③ 核心主体 从 desc 来
        # ④ 场景自适应环境
