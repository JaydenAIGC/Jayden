...
class Backend:
    ...
    def gen_subject_ideas(self):
        try:
            ...
            _land=_idea_r.choice(["平缓草甸","平缓草甸","辽阔草原","辽阔草原","薄雾丘陵","薄雾丘陵","晨雾旷野","晨雾旷野","雾霭林地","雾霭林地","浅雾原野","薄雾山麓","寂静湖面","静谧湖畔","暮色荒滩","朦胧海岸","云海","沙漠","火山口","积雪丘陵"])
...
只写【核心主体+配套环境】，80-120字。可以包含光线时段（黄昏/清晨/正午/阴天），但不要写画风、镜头、构图、人物、画质——那些由出图系统统一添加。
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        # ⑥ 光影色彩 — 若描述词已含光线时段则跳过
        has_light=any(w in desc for w in ["黄昏","清晨","正午","阴天","晨光","暮色","夕阳","日出","日落","午后","黎明","深夜","月光"])
        if not has_light:
