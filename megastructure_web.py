...
**人物规则（特殊处理）：**
① 如果原始文字已经存在人物、穿搭、动作描述 → 一字遵循设定，不可随意修改
② 如果原始文字不存在人物 → 结合场景氛围自主匹配风格统一的单人形象（如骑马/古风长袍/科幻战甲/探险装束），放置画面下方远景，体型渺小，人物不能占据视觉中心
③ 硬性限制：仅限单人，远景，不特写，不生成人群
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        # ⑤ 人物自适应策略
        neg_human=""
        has_char=any(w in desc for w in ["人影","剪影","人物","行人","身影","背影","骑手","人","女","男","战士","骑","袍","甲","衣","帽","剑","弓"])
        if has_char:
            pass  # 原始已有人物描述，一字不修改，信任原文
        elif human:
            env+=f"，画面底部远景仅1个人类剪影，背对镜头眺望主体，体型渺小无面部细节，仅尺度参照不占视觉重心"
