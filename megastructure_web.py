...
- ⚠️ **核心美学逻辑**：柔和弥散、边界渐变的均质自然基底，对冲轮廓锐利、秩序严谨的冰冷几何巨构，全部美感来源于此质感对冲
- 完整保留以下主体：{subject}
- 焦距24-35mm广角，三大视角循环：低角度仰拍/45度斜向俯视/高空远景平视
- 两种布局交替：准对称居中构图（环形/穹顶类）或三分法偏移构图（留白40-65%）
- 干净两点透视，无扭曲畸变
- 人物仅放画面底部边缘下三分区域，1个渺小剪影，不遮挡主体轮廓
- 画面层次仅三层：稀薄前景→中景主体→远景朦胧环境
- **氛围核心：辽阔静谧、超然孤寂。均质连续型广阔自然基底（流体：云海/雾霭/静水；固态：雪原/戈壁/平缓均质草甸），整体连贯无零散杂物无完整生态系统。巨型无机几何构造独立于自然体系，二者互不交融。排斥丰富生态与生机场景，允许平整草甸/雪原/云海/深海/戈壁（远景融入雾气），禁止野花/树丛/乱石/峡谷/丛林/起伏山地/动植物**
- 全程用中文"""
        else:
...
- ⚠️ 核心审美：环境柔软混沌 vs 建筑硬朗清晰，质感对冲是画面全部美感来源
- 自主创作宏大核心主体，单一核心不堆砌
- 焦距24-35mm广角，三大视角循环，准对称/三分法构图交替
- 人物仅放底部边缘，1个渺小剪影，不遮挡主体
- 画面层次仅三层，**氛围：辽阔静谧，均质连续自然基底(云海/雾霭/雪原/戈壁/平整草甸) + 独立于自然体系的无机几何巨构，排斥丰富生态生机场景，禁止零散杂物枝叶植被**
- 全程用中文"""
        try:
            h=self._headers("llm")
            p={"model":self.config.get("text_model","deepseek-v4-flash"),
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        # ② 画风标签
        if is_natural:
            style_tag="影视环境概念原画，Octane渲染，写实物理渲染，低纹理密度，哑光平滑材质，剔除繁杂细碎雕刻，自然光线，低饱和度统一色调，画面通透干净"
        else:
            style_tag="影视环境概念原画，Octane渲染，写实物理渲染，极简巨型几何构造，依靠嵌套、悬挑、中空、层级落差塑造立体感，低纹理密度，哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，柔和弥散均质自然基底对冲轮廓锐利冰冷几何巨构，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景，仅渺小单人作尺度参照，8K超清"
        # ③ 核心主体 从 desc 来
        # ④ 场景自适应环境
        if any(w in d for w in ["海","海洋","海岸","沙滩","海滩","礁石","浪"]):
            env="，主体坐落于海岸边，宽阔平静海面作为大面积留白，远处海平线融入薄雾，无船只无海鸟无沙滩杂物"
        elif any(w in d for w in ["森林","林","树","丛林","密林","林地"]):
            env="，主体立于平缓均质草甸之上，远方薄雾弥漫，远景融入雾气，无零散树木无灌木无植被"
            ...
            env="，主体悬浮于通透渐变海水中，开阔水体作为大面积留白，自上而下海面透射自然光束，水体清澈微光薄雾，无鱼群无水草无贝壳碎石"
        elif any(w in d for w in ["城市","都市","赛博","霓虹","街道","高楼"]):
            env="，主体耸立于城市之中，平整路面与纯净天空作为留白，干净简洁建筑轮廓，无行人无车辆无广告牌"
        elif any(w in d for w in ["古","中式","东方","庙","塔","宫","殿","亭","阁"]):
            env="，主体取东方抽象轮廓，提炼重檐/坡顶几何形态并完全简化，无斗拱瓦片雕花木质结构，哑光岩体完整连续，大面积薄雾留白，远景融入雾气"
        elif any(w in d for w in ["雪原","冰原","冻土","苔原"]):
            env="，主体立于无垠雪原之上，平整雪地作为大面积留白，远处薄雾地平线，无任何生物痕迹"
        else:
