...
- **氛围核心：空灵静谧，万物静止无声，无活跃生命迹象（无飞鸟无走兽无流水），但不是末日废墟，不是灰暗灾难，画面干净通透有呼吸感**
- 全程用中文"""
        else:
...
- 画面层次仅三层，**氛围核心：空灵静谧，万物静止无声，无活跃生命迹象，但不是末日废墟，画面干净通透有呼吸感**
- 全程用中文"""
        try:
            h=self._headers("llm")
            p={"model":self.config.get("text_model","deepseek-v4-flash"),
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        if is_natural:
            ...
            style_tag="影视环境概念原画，Octane渲染，写实物理渲染，极简巨型几何构造，依靠嵌套、悬挑、高低层级塑造层次感，低纹理密度，哑光平滑材质，剔除繁杂细碎雕刻、密集零件，单一柔和漫射天光，低饱和度统一色调，画面大面积留白，强烈人与巨构尺度反差，空灵静谧氛围，万物静止无声，无活跃生命迹象，不是末日废墟不是灰暗灾难，轮廓干净利落，8K超清，核心审美：柔软混沌环境包裹硬朗清晰几何巨构，质感对冲"
        # ③ 核心主体 从 desc 来
        # ④ 场景自适应环境
