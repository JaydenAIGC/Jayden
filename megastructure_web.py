...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        if is_natural:
            ...
            style_tag="美术概括化的3A写实游戏场景CG静帧，遵循PBR三维渲染逻辑；写实仅作为基底，主动概括简化细节脱离照片真实感；无手绘笔触，依靠体积雾与统一色调塑造辽阔静谧氛围；低纹理密度哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，画面拥有无限延伸感——环境边界渐变虚化消融于雾霭，无硬质隔断，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景，画面干净克制"
        # ③ 核心主体 从 desc 来
