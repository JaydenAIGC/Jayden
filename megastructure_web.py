...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        # ② 画风标签
        if is_natural:
            style_tag="影视环境概念原画，Octane渲染，可信写实物理材质，低纹理密度哑光材质，自然光线柔和过渡，低饱和度统一色调，大范围体积雾塑造空间层次，画面干净通透"
        else:
            style_tag="影视环境概念原画，Octane渲染，可信写实物理材质（非极致写实），极简巨型几何构造，依靠嵌套、悬挑、中空、层级落差塑造立体感，低纹理密度哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，画面拥有无限延伸感——环境边界渐变虚化消融于雾霭，无硬质隔断，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景"
        # ③ 核心主体 从 desc 来
