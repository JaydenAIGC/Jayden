...
- 完整保留以下主体：{subject}
- **环境：均质连续型广阔自然基底（云海/雾霭/静水/雪原/戈壁/平缓草甸），整体连贯无零散杂物，远景融入雾气，无野花树丛乱石峡谷丛林动植物**
- 全程用中文"""
        else:
...
- 自主创作宏大核心主体，单一核心不堆砌
- **环境：均质连续自然基底(云海/雾霭/雪原/戈壁/平整草甸)，无零散杂物，远景融入雾气**
- 全程用中文"""
        try:
            h=self._headers("llm")
            p={"model":self.config.get("text_model","deepseek-v4-flash"),
...
只输出JSON数组，不要多余文字"""}],
    ...
    def build_prompt(self,desc,comp=False,light=False,human=True,size="1024x1792"):
        ...
        if is_natural:
            ...
            style_tag="影视环境概念原画，Octane渲染，写实物理渲染，极简巨型几何构造，依靠嵌套、悬挑、中空、层级落差塑造立体感，低纹理密度，哑光平滑材质，无表面雕花无密集孔洞无外挂零件，单一柔和漫射天光，低饱和度统一色调，柔和弥散均质自然基底对冲轮廓锐利冰冷几何巨构，辽阔静谧超然孤寂，巨构独立于自然体系互不交融，排斥丰富生态生机场景，8K超清"
        # ③ 核心主体 从 desc 来
        # ④ 场景自适应环境
